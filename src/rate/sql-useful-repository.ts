...
export class SqlUsefulRateRepository implements UsefulRateRepository {
    ...
    map?: StringMap;
    getUseful(id: string, author: string, userId: string, ctx?: any): Promise<UsefulRate | null> {
        return this.db.query<UsefulRate>(`select * from ${this.table} where id = ${this.db.param(1)} and author = ${this.db.param(2)} and userId = ${this.db.param(3)}`, [id, author, userId], this.map, undefined, ctx).then(rates => {
